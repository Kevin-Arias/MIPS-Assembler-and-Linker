...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
    ...
    while(fgets(buf, BUF_SIZE, input)) {
      ...
      int label = add_if_label(input_line, name, byte_offset, symtbl);
      if (label == -1) {
        ret_code = -1;
        continue;
      } else if (label == 1) {
      ...
      while (token != NULL) {
        args[num_args] = token;
        //I get a weird error when using parse_args. It says I
        //need a int* num_args when I was provided just an int.
